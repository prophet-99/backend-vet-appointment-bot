...
export class ConversationService {
  ...
  private newState(conversationId: string): BookingState {
    ...
    return {
      ...
      statusMode: FlowStatusMode.INITIAL,
      statusAI: FlowStatusAI.COLLECTING,
      showGreeting: true,
      expiresAt,
      lastUserText: '',
      lastBotText: '',
  ...
  async handleChatTurn(params: {
    ...
    if (params.userSelectionId) {
      ...
      state.statusMode = FlowStatusMode.INITIAL;
      state.statusAI = FlowStatusAI.COLLECTING;
    }
    ...
    if (userIntent === FlowMode.WELCOME) {
      ...
      if (state.statusMode === FlowStatusMode.INITIAL) {
        stateToPatch.mode = FlowMode.WELCOME;
        stateToPatch.statusMode = FlowStatusMode.IN_PROGRESS;
      ...
      if (state.statusMode === FlowStatusMode.IN_PROGRESS) {
    ...
    if (userIntent === FlowMode.CREATE) {
      ...
      if (state.statusMode === FlowStatusMode.IN_PROGRESS) {
        ...
        if (aiResponse?.aiStatus === FlowStatusAI.DONE) {
          stateToPatch.statusMode = FlowStatusMode.COMPLETED;
      ...
      if (state.statusMode === FlowStatusMode.COMPLETED) {
        stateToPatch.lastBotText = MENU_SELECTION_REQUIRED_MESSAGE;
        // TODO: DELETE THIS COMMENT:
        /**
         *! FLUJO DE ERROR - USUARIO NO SELECCIONA UNA OPCIÓN VÁLIDA DEL MENÚ
