  ...
  async handleChatTurn(params: {
    ...
    if (userIntent === 'CREATE') {
      ...
      if (state.statusMode === 'IN_PROGRESS') {
        ...
        if (aiResponse.flowCompleted) {
          stateToPatch.statusMode = 'COMPLETED';
        }

        stateToPatch.lastBotText = aiResponse.text.trim();
        stateToPatch.expiresAt = this.calculateBookingExpiration();

        stateToPatch = patchBookingState(stateToPatch, aiResponse.statePatch);
      }
    ...
    if (userIntent === 'DELETE') {
      ...
      if (state.statusMode === 'IN_PROGRESS') {
        // TODO: IA para inferir y obtener el código de reserva
        //? Luego obteniendolo llamo a la función de schedulare para cancelar
        //? Si no se puede inferir el código de reserva, se le pide al cliente que lo ingrese directamente
        if (true) {
          // Condición si tiene el codigo de reserva
          //? Llamar a la función de cancelación del scheduler con el código de reserva
          // Si es exito:
          stateToPatch.lastBotText =
            'Tu cita ha sido cancelada exitosamente. Si necesitas algo más, no dudes en contactarnos. ¡Que tengas un buen día! 🐾';
          stateToPatch.statusMode = 'COMPLETED';
          // Si no es exito:
          stateToPatch.statusMode = 'ERROR';
        }
      }
